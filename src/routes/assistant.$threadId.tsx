...
function AssistantPage() {
  ...
  async function onAsk() {
    ...
    try {
      ...
      const residentResults = executeResidentCalls(
      ...
      persist([
        ...withUser,
        {
          ...
          residentResults: residentResults.length ? residentResults : undefined,
        },
      ]);
      ...
      for (const r of residentResults) {
        logAiActivity({
          ...ctx,
          kind: "execute_action",
          title: `Resident: ${r.tool}`,
          detail: r.output.slice(0, 400),
  ...
  return (
    <AppShell
      ...
      subtitle={
        accountLabel
          ? `JARVIS · ${accountLabel} · residente + mani`
          : "JARVIS residente — SOUL · memoria · skill · mani"
      }
    >
      <div className="grid gap-4 p-4 lg:grid-cols-[200px_1fr_1fr] lg:p-6">
        ...
        <section className="panel flex max-h-[70vh] flex-col">
          <div className="flex flex-wrap items-center gap-2 border-b border-border px-3 py-2">
            ...
            <span className="text-[11px] uppercase tracking-wider text-muted-foreground">Chat</span>
            <select
              value={model}
              onChange={(e) => onModelChange(e.target.value)}
              className="ml-auto rounded border border-border bg-background px-2 py-1 text-[11px]"
            >
              {GROQ_MODELS.map((m) => (
                ...
                {m.residentResults?.length ? (
                  <div className="mt-2 space-y-1.5">
                    ...
                    {m.residentResults.map((r, ri) => (
                      ...
                      >
                        <span className="font-mono text-primary">{r.tool}</span>
                        <span className="text-muted-foreground"> — </span>
                        <span className="whitespace-pre-wrap">{r.output.slice(0, 500)}</span>
                      </div>
                    ))}
                  </div>
                ) : null}
                ...
                {m.actions?.map((a, ai) => {
                  ...
                  return (
                    <div key={ai} className="mt-2 rounded border border-border p-2">
                      ...
                      <p className="text-[10px] text-muted-foreground">{riskLabel(risk)}</p>
                      {a.state === "pending" ? (
                        <button
          ...
          <div className="border-t border-border p-3">
            <div className="flex gap-2">
              <textarea
                ...
                rows={2}
                placeholder="Scrivi a JARVIS… (Enter invia)"
                className="min-h-[48px] flex-1 resize-none rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary"
                disabled={busy}
              />
              <button
                type="button"
                ...
                className="btn-primary inline-flex h-12 w-12 shrink-0 items-center justify-center"
                aria-label="Invia"
              >
          ...
          <section className="panel space-y-2 p-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="comando console"
                className="flex-1 rounded border border-border bg-background px-2 py-1.5 font-mono text-xs"
              />
              <button
                type="button"
                onClick={() => void onSendCommand()}
                className="btn-matrix rounded border border-primary px-2 py-1 text-[10px] uppercase text-primary"
              >
                run
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              <select
                ...
                className="max-w-full rounded border border-border bg-background px-2 py-1 text-[11px]"
              >
                {actionOptions.map((o) => (
            ...
            <pre className="max-h-28 overflow-auto rounded border border-border bg-black/30 p-2 font-mono text-[10px] text-muted-foreground">
              {consoleOut.slice(-20).join("\n") || "—"}
            </pre>
          </section>
        </div>
      </div>
